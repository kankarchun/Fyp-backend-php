<?php
	//invoice
	function getFoodProfit($access_token,$restID){
	...
	//invoice
	function getTimeOrder($access_token,$restID){
		require_once("../library/connections/conn.php");
	...
	//invoice
	function getAllRestaurantProfit($access_token,$companyID){
		require_once("../library/connections/conn.php");
		...
		if(mysqli_num_rows($rs)<=0){
			...
			if($rc['expireDate']<$current){
				...
				foreach($lastMonths as $month){
					...
					$sql="select * from invoice where orderDateTime between Date('$firstDayMonth') and Date('$lastDayMonth') and restID in (select restID from restaurant where companyID='$companyID')";
					$rs=mysqli_query($conn,$sql) or die(mysqli_error($conn));
					$total=0;
					while($rc=mysqli_fetch_assoc($rs)){
						$total+=$rc['totalCost'];
	...
	function getRestaurantProfit($access_token,$restID){
		require_once("../library/connections/conn.php");
